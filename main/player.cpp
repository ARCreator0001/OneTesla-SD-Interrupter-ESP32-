...
void note_on(unsigned char pitch, unsigned char value) {
  ...
  if (note1->pitch == pitch) { make_note(note1, pitch, value); retrigger(); return; }
  if (note2->pitch == pitch) { make_note(note2, pitch, value); retrigger(); return; }

  if (note1->pitch == 0) { make_note(note1, pitch, value); retrigger(); return; }
  if (note2->pitch == 0) { make_note(note2, pitch, value); retrigger(); return; }
...
void note_off(unsigned char pitch) {
  if (note1->pitch == pitch) { note1->pitch = 0; retrigger(); return; }
  if (note2->pitch == pitch) { note2->pitch = 0; retrigger(); return; }
}
...
void control_change(unsigned char cmd, unsigned char value) {
  if (cmd == 0x78 || cmd == 0x79 || cmd == 0x7B || cmd == 0x7C) note_stop();
}

void retrigger() {
  if (note1->pitch > 0) startTimer1(); else disengageISR1();
  if (note2->pitch > 0 && note2->period < TIMER_2_PERIOD_MAX) startTimer2(); else disengageISR2();
}
